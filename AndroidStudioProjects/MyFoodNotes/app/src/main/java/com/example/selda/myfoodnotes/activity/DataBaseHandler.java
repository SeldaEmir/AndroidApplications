...
public class DataBaseHandler extends SQLiteOpenHelper {
    ...
    public int getCountProduct(){
       ...
       public PlacesModel getPlace(String id){
        ...
        Cursor cursor=sqLiteDatabase.query(tablePlaces,new String[]{keyPlacesId,
                keyPlacesImagePath,keyPlacesName,
                keyPlacesLocLang,keyPlacesLocLong,
                keyPlacesLikeDislike},keyPlacesId+"=?",new String[]{id},null,null,null,null);
        if(cursor!=null)
           cursor.moveToFirst();
    ...
    public ProductModel getProductById(String id) {
        ProductModel productt = new ProductModel();
        SQLiteDatabase db = this.getReadableDatabase();
        String selectQuery = "select * from " + tableProducts + " where " + keyProductId + "=" + id;
        Cursor cursor = db.rawQuery(selectQuery, null);
        if (cursor.moveToFirst()) {

            productt.setProduct_id(cursor.getInt(0));
            productt.setPlaces_id(cursor.getInt(1));
            productt.setProduct_date(cursor.getString(2));
            productt.setProduct_image_path(cursor.getString(3));
            productt.setProduct_name(cursor.getString(4));
            productt.setProduct_price(cursor.getString(5));
            productt.setProduct_like_dislike(cursor.getString(6));
    ...
    public List<ProductModel> getAllProductsByPlaceId(String id){
        List<ProductModel>productsModelList=new ArrayList<>();
        SQLiteDatabase db=this.getReadableDatabase();
        String selectQuery="select * from "+tableProducts+" where "+keyProductPlacesId+"="+id;
        Cursor cursor=db.rawQuery(selectQuery,null);
        if(cursor.moveToFirst())
            do {
                ProductModel productModel=new ProductModel();
...
public List<ProductCommentModel> getAllComments(){
    ...
    public List<ProductCommentModel> getCommentsByProductId(String productId) {
        List<ProductCommentModel> comments = new ArrayList<>();
        SQLiteDatabase db = getReadableDatabase();
        String selectQuery = "select * from " + tableProductComment + " where " + keyCommentProductId + "=" + productId;
        Cursor cursor = db.rawQuery(selectQuery, null);
        /*
                String createTableCommentProduct = "CREATE TABLE " + tableProductComment + "(" +
                keyCommentProductId + " INTEGER," +
        ...
        if (cursor.moveToFirst()) {
            do {
                ProductCommentModel model = new ProductCommentModel(cursor.getInt(0), cursor.getString(1), cursor.getString(2), cursor.getString(3));
                comments.add(model);
            } while (cursor.moveToNext());
        }
